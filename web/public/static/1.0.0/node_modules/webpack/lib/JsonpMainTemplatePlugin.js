define('webpack/lib/JsonpMainTemplatePlugin', function(require, exports, module) {
  ...
  */
  ...
  JsonpMainTemplatePlugin.prototype.apply = function (mainTemplate) {
  	...
  	mainTemplate.plugin("jsonp-script", function (_, chunk, hash) {
  		...
  		return this.asString(["var script = document.createElement('script');", "script.type = 'text/javascript';", "script.charset = 'utf-8';", "script.async = true;", crossOriginLoading ? "script.crossOrigin = '" + crossOriginLoading + "';" : "", "script.src = " + this.requireFn + ".p + " + this.applyPluginsWaterfall("asset-path", JSON.stringify(chunkFilename), {
  	...
  	mainTemplate.plugin("hot-bootstrap", function (source, chunk, hash) {
  		...
  		return source + "\n" + "var parentHotUpdateCallback = this[" + JSON.stringify(hotUpdateFunction) + "];\n" + "this[" + JSON.stringify(hotUpdateFunction) + "] = " + Template.getFunctionContent(require("webpack/lib/JsonpMainTemplate.runtime")).replace(/\$require\$/g, this.requireFn).replace(/\$hotMainFilename\$/g, currentHotUpdateMainFilename).replace(/\$hotChunkFilename\$/g, currentHotUpdateChunkFilename).replace(/\$hash\$/g, JSON.stringify(hash));
  	});
