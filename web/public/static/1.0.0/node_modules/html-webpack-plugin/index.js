define('html-webpack-plugin/index', function(require, exports, module) {

  var global = typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {};
  'use strict';
  var vm = require('vm');
  var fs = require('fs');
  var _ = require('babel-core/node_modules/lodash/lodash');
  var Promise = require('html-webpack-plugin/node_modules/bluebird/js/browser/bluebird');
  var path = require('path');
  var childCompiler = require('html-webpack-plugin/lib/compiler');
  var prettyError = require('html-webpack-plugin/lib/errors');
  var chunkSorter = require('html-webpack-plugin/lib/chunksorter');
  Promise.promisifyAll(fs);
  
  function HtmlWebpackPlugin(options) {
    // Default options
    this.options = _.extend({
      template: path.join("/node_modules/html-webpack-plugin", 'default_index.ejs'),
      filename: 'index.html',
      hash: false,
      inject: true,
      compile: true,
    ...
    compiler.plugin('emit', function (compilation, callback) {
      ...
      .then(function (html) {
        ...
        return applyPluginsAsyncWaterfall('html-webpack-plugin-alter-asset-tags', true, pluginArgs).then(function (result) {
          ...
          return self.postProcessHtml(html, assets, { body: result.body, head: result.head }).then(function (html) {
  ...
  HtmlWebpackPlugin.prototype.postProcessHtml = function (html, assets, assetTags) {
    ...
    .then(function (html) {
      if (self.options.minify) {
        var minify = require('html-webpack-plugin/node_modules/html-minifier/src/htmlminifier').minify;
  ...
  HtmlWebpackPlugin.prototype.createHtmlTag = function (tagDefinition) {
    ...
    return '<' + [tagDefinition.tagName].concat(attributes).join(' ') + (tagDefinition.selfClosingTag ? '/' : '') + '>' + (tagDefinition.innerHTML || '') + (tagDefinition.closeTag ? '</' + tagDefinition.tagName + '>' : '');
  };
  ...
  HtmlWebpackPlugin.prototype.getFullTemplatePath = function (template, context) {
    ...
    return template.replace(/([!])([^\/\\][^!\?]+|[^\/\\!?])($|\?.+$)/, function (match, prefix, filepath, postfix) {
