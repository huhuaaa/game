define('extract-text-webpack-plugin/index', function(require, exports, module) {
  ...
  */
  ...
  var ConcatSource = require("extract-text-webpack-plugin/node_modules/webpack-sources/lib/index").ConcatSource;
  var async = require("extract-text-webpack-plugin/node_modules/async/lib/async");
  var ExtractedModule = require("extract-text-webpack-plugin/ExtractedModule");
  var Chunk = require("webpack/lib/Chunk");
  var OrderUndefinedError = require("extract-text-webpack-plugin/OrderUndefinedError");
  var loaderUtils = require("babel-loader/node_modules/loader-utils/index");
  ...
  ExtractTextPlugin.extract = function (before, loader, options) {
  	if (typeof loader === "string" || Array.isArray(loader)) {
  		...
  		return [ExtractTextPlugin.loader(mergeOptions({ omit: before.length, extract: true, remove: true }, options))].concat(before, loader).join("!");
  	} else {
  		options = loader;
  		loader = before;
  		return [ExtractTextPlugin.loader(mergeOptions({ remove: true }, options))].concat(loader).join("!");
  	}
  };
  ...
  ExtractTextPlugin.prototype.extract = function (before, loader, options) {
  	if (typeof loader === "string" || Array.isArray(loader)) {
  		...
  		return [this.loader(mergeOptions({ omit: before.length, extract: true, remove: true }, options))].concat(before, loader).join("!");
  	} else {
  		options = loader;
  		loader = before;
  		return [this.loader(mergeOptions({ remove: true }, options))].concat(loader).join("!");
  	}
  };
  ...
  ExtractTextPlugin.prototype.apply = function (compiler) {
  	...
  	compiler.plugin("this-compilation", (function (compilation) {
  		...
  		compilation.plugin("normal-module-loader", function (loaderContext, module) {
  			loaderContext["/node_modules/extract-text-webpack-plugin"] = function (content, opt) {
  				if (options.disable) return false;
  				if (!Array.isArray(content) && content !== null) throw new Error("Exported value is not a string.");
  				module.meta["/node_modules/extract-text-webpack-plugin"] = {
  					content: content,
  					options: opt || {}
  				};
  				return options.allChunks || module.meta["/node_modules/extract-text-webpack-plugin" + "/extract"]; // eslint-disable-line no-path-concat
  			};
  		});
  		...
  		compilation.plugin("optimize-tree", (function (chunks, modules, callback) {
  			...
  			async.forEach(chunks, function (chunk, callback) {
  				...
  				async.forEach(chunk.modules.slice(), function (module, callback) {
  					var meta = module.meta && module.meta["/node_modules/extract-text-webpack-plugin"];
  					if (meta && (!meta.options.id || meta.options.id === id)) {
  						var wasExtracted = Array.isArray(meta.content);
  						if (shouldExtract !== wasExtracted) {
  							module.meta["/node_modules/extract-text-webpack-plugin" + "/extract"] = shouldExtract; // eslint-disable-line no-path-concat
  							compilation.rebuildModule(module, function (err) {
  								if (err) {
  									compilation.errors.push(err);
  									return callback();
  								}
  								meta = module.meta["/node_modules/extract-text-webpack-plugin"];
  				...
  				extractedChunks.forEach(function (extractedChunk) {
  					if (extractedChunk.initial) this.mergeNonInitialChunks(extractedChunk);
  				}, this);
  				extractedChunks.forEach(function (extractedChunk) {
